...
function header(doc, logoData) {
    ...
    doc.rect(0, 0, W, 32, 'F')
    if (logoData) {
          doc.addImage(logoData, 'PNG', M, 4, 22, 22)
    }
    doc.setFontSize(16); doc.setFont('helvetica', 'bold'); doc.setTextColor(255, 255, 255)
    doc.text('LIMPLINE COMERCIAL', M + 26, 13)
    doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.setTextColor(200, 220, 255)
    doc.text('Dispensers · Papéis · Comodato', M + 26, 19)
    doc.setFontSize(8); doc.setTextColor(255, 255, 255)
    doc.text('(11) 2335-3500  |  limplinecomercial.com.br  |  vendas@limplinecomercial.com.br', W - M, 26, { align: 'right' })
...
export async function generateProposta(data) {
  ...
  if (data.produtos && data.produtos.length) {
        ...
        data.produtos.forEach((it, i) => {
                if (y > 265) { footer(doc, doc.internal.getNumberOfPages()); doc.addPage(); header(doc, logoData); y = 36 }
...
function generateContratoPages(doc, data, logoData) {
  ...
  clausulas.forEach(cl => {
        if (y > 240) { footer(doc, doc.internal.getNumberOfPages()); doc.addPage(); header(doc, logoData); y = 36 }
        doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(...AZUL_M)
  ...
  if (y > 220) { footer(doc, doc.internal.getNumberOfPages()); doc.addPage(); header(doc, logoData); y = 36 }
    y += 8
    doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(...CINZA)
