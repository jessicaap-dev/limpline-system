...
export default function Proposta() {
  ...
  async function handleGerar() {
    ...
    setLoading(true)
    try {
      const data = { ...cliente, comodato, produtos: items, vendedora: user.name, genero: user.genero, showTotal }
      ...
      try {
        ...
        if (insertError) console.error('Erro histÃ³rico:', insertError)
      } catch (e) {
  ...
  return (
    <Layout title="Gerar Proposta">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        {['cliente', 'comodato', 'produtos', 'resumo'].map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'cliente' ? '1. Cliente' : t === 'comodato' ? '2. Comodato' : t === 'produtos' ? '3. Produtos' : '4. Gerar PDF'}
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          <div style={{ marginTop: '1.5rem', marginBottom: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: '#1A3A6B' }}>Produto nÃ£o estÃ¡ na lista?</span>
            ...
            {customProducts.map(cp => (
              <div key={cp.id} style={{ border: '0.5px solid #E8EDF5', borderRadius: 10, padding: '10px 12px', marginBottom: 8, background: '#FFFBF0' }}>
                <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                  <input placeholder="DescriÃ§Ã£o do produto" value={cp.name} onChange={e => updateCustomProduct(cp.id, 'name', e.target.value)}
